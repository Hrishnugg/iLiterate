...
describe("FlashcardCard", () => {
  it("shows front content and reveals pronunciation and hint", () => {
    ...
    expect(screen.getByText("Show pronunciation")).toBeInTheDocument();
    expect(screen.getByText("Show context")).toBeInTheDocument();

    // show pronunciation
    fireEvent.click(screen.getByText("Show pronunciation"));
    ...
    // clicking the card calls onFlip
    fireEvent.click(screen.getByText(/Tap or press Space to reveal/i));
  ...
  it("renders back side when flipped", () => {
    ...
    expect(screen.getByText(/Reviewed 3x/)).toBeInTheDocument();
  });
});
